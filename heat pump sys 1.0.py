...
#open cop yaml file
file_path1 = #your yaml file path in your computer
...
class Heat_system:
    def __init__(self,Aw,Uw,Ar,Ur,T_amb_list,T_sp,U_cond,T_cond,U_tank,c_t):#M_water,Ini_tank_temp,pump_critemp,Q_loss,U_tank,A_tank
        self.Aw = Aw
        self.Uw = Uw
        self.Ar = Ar
        self.Ur = Ur
    ...
    def Q_hp(self,T_tank):
        ...
        if self.pump_on:
            return self.U_cond*(self.Aw + self.Uw)*(self.T_cond - T_tank)
        else:
            return 0
    ...
    def tank_temperature_ode(self, t, T_tank):
       ...
       # 确保t的索引不超过T_amb_list的范围
        ...
        T_amb = self.T_amb_list[index]  # 从环境温度列表中获取当前温度
   ...
   # Solve_ivp 使用
    def solve_tank_temperature(self, initial_tank_temp, total_time=86400, time_points=1000):
       ...
       # 提取结果
        T_tank_values = solution.y[0]  # 水箱温度的解
